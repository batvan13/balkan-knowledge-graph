<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-sm rounded-lg p-6">
                <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Price Signals</h3>

                @if($entity->priceSignals->isNotEmpty())
                    <table class="w-full text-sm text-left mb-6">
                        <thead class="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase text-xs tracking-wide">
                            <tr>
                                <th class="px-3 py-2">Signal Type</th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @foreach($entity->priceSignals->sortByDesc('observed_at') as $signal)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-3 py-2 font-mono text-gray-700">{{ $signal->signal_type }}</td>
                                    <td class="px-3 py-2 text-gray-700">{{ $signal->price_category ?? '—' }}</td>
                                    <td class="px-3 py-2 font-mono text-gray-500">{{ $signal->currency ?? '—' }}</td>
                                    <td class="px-3 py-2 text-gray-900">{{ $signal->amount_min !== null ? number_format($signal->amount_min, 2) : '—' }}</td>
                                    <td class="px-3 py-2 text-gray-900">{{ $signal->amount_max !== null ? number_format($signal->amount_max, 2) : '—' }}</td>
                                    <td class="px-3 py-2 text-gray-400">{{ $signal->observed_at?->format('Y-m-d H:i') ?? '—' }}</td>
                                    <td class="px-3 py-2 text-right">
                                        <a href="{{ route('admin.entities.price-signals.edit', [$entity, $signal]) }}"
                                           class="text-blue-600 hover:underline text-sm">Edit</a>
                                    </td>
                                </tr>
                            @endforeach
                ...
                @else
                    <p class="text-sm text-gray-400 mb-6">No price signals yet.</p>
                @endif

                <div class="border-t border-gray-100 pt-5">
                    <h4 class="text-sm font-medium text-gray-600 mb-4">Add Price Signal</h4>

                    <form method="POST" action="{{ route('admin.entities.price-signals.store', $entity) }}">
                        @csrf
                            ...
                            <div>
                                ...
                                <select name="signal_type"
                                    ...
                                    @foreach(['observed', 'owner_declared'] as $st)
                                        <option value="{{ $st }}" @selected(old('signal_type') === $st)>{{ $st }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="price_category"
                                    ...
                                    @foreach(['budget', 'midrange', 'premium', 'luxury'] as $pc)
                                        <option value="{{ $pc }}" @selected(old('price_category') === $pc)>{{ $pc }}</option>
                                    @endforeach
                                </select>
                        ...
                        <p class="text-xs text-gray-400 mb-4">At least one of price category, amount min, or amount max is required. Currency is required when amounts are specified.</p>
